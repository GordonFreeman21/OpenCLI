    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in opencli.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .opencli/ subdirectory too)
  const candidates = [
    path.join(baseDir, "opencli.json"),
    path.join(baseDir, "opencli.jsonc"),
  ...
  if (!global) {
    candidates.push(
      path.join(baseDir, ".opencli", "opencli.json"),
      path.join(baseDir, ".opencli", "opencli.jsonc"),
      path.join(baseDir, ".opencli", "opencode.json"),
      path.join(baseDir, ".opencli", "opencode.jsonc"),
    )
  }
  ...
  // Default to opencli.json if none exist
  return candidates[0]
}
