...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run opencli with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("model", {
        type: "string",
        alias: ["m"],
        describe: "local model to use in the format provider/model",
      })
      .option("agent", {
        type: "string",
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running opencli server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
