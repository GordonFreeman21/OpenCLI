...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opencli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall opencli")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opencli",
      pnpm: "pnpm uninstall -g opencli",
      bun: "bun remove -g opencli",
      yarn: "yarn global remove opencli",
      brew: "brew uninstall opencli",
      choco: "choco uninstall opencli",
      scoop: "scoop uninstall opencli",
    }
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".opencli/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opencli/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".opencli/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".opencli"))
    ) {
      continue
    }
