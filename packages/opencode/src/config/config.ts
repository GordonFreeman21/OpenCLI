...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/opencli"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "opencli")
      default:
        return "/etc/opencli"
    }
  }
  ...
  const MANAGED_PLIST_DOMAIN = "ai.opencli.managed"
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.opencli/command/", "/.opencli/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.opencli/agent/", "/.opencli/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: opencli.local)"),
  ...
  export const Info = z
    .object({
      ...
      model: ModelId.describe("Local model to use, for example ollama/llama3.2 or lmstudio/qwen2.5-coder").optional(),
      small_model: ModelId.describe("Small local model to use for tasks like title generation").optional(),
      default_agent: z
        .string()
        .optional()
        .describe(
  ...
  function globalConfigFile() {
    const candidates = ["opencli.jsonc", "opencli.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "opencli.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "opencli.jsonc"))),
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          for (const dir of unique(directories)) {
            if (dir.endsWith(".opencli") || dir === Flag.OPENCODE_CONFIG_DIR) {
              for (const file of ["opencli.json", "opencli.jsonc", "opencode.json", "opencode.jsonc"]) {
